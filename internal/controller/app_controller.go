...
type AppReconciler struct {
	// 这里 Client 的作用比较清晰，就是在 CRD Controller 执行协调的过程中，需要通过 ClientCRUD(Create、Retrieve、Update、Delete)CRD，即 Get、Create 等方法。所以 DemoReconciler 的结构体第一个元素的对象，指向的是 Controller-runtime 包中的 Client 接口对象，它设计了必要的方法，如 Get、List、Update 等
	client.Client
...
// Reconciler(协调器)是提供给 Controller 的一个函数，可以随时使用对象的 Name 和 Namespace 对其进行调用。当它被调用时，Reconciler 将确保集群中资源的状态和 预设的状态保持一致。例如，ReplicaSet 指定 5 个副本，但系统中仅存在 3 个 Pod 时， Reconciler 将再创建 2 个 Pod，并向 Pod 的 OwnerReference 中添加该 ReplicaSet 的名 称，同时设置“controller=true”属性。
// Reconciler 需要开发者自己实现，并在创建 Controller 时，通过 Builder.Complete() 或 Bu- ilder.Build() 方法传递给 Controller。Reconciler 接口定义在 pkg/reconcile/reconcile.go 下， 只有一个该方法:Reconcile(context.Context，Request) (Result，error)。
...
//
// type Result struct {
// // Requeue 告诉 Controller 是否需要重新将对象加入队列，默认为 False Requeue bool
// // RequeueAfter 大于 0 表示 Controller 需要在设置的时间间隔后，将对象重新加入队列
	...
	if app.ObjectMeta.DeletionTimestamp.IsZero() {
		...
		if containsString(app.ObjectMeta.Finalizers, myFinalizerName) {
			...
			if err := r.deleteExternalResources(app); err != nil {
				// 如果删除失败，则直接返回对应的 err，Controller 会自动执行重试逻辑
				return ctrl.Result{}, err
			}
...
// CRD 的 Controller 初始化的核心代码是 SetupWithManager 方法，借助这个方法，就可以完成 CRD 在 Manager 对象中的安装，最后通过 Manager 对 象的 start 方法来完成 CRD Controller 的运行
// SetupWithManager 使用 Manager 设置 Controller
// 它首先借助 Controller-runtime 包初始化 Builder 对象，当它完成 Complete 方法时，实 际完成了 CRD Reconciler 对象的初始化，而这个对象是一个接口方法，它必须实现 Reconcile 方法。
// ctrl.NewControllerManagedBy 方法实际借助 Controller-runtime 完成了 Builder 对象的构建，并借助（Complete）关联 CRD API 定 义的 Scheme 信息，从而得知 CRD 的 Controller 需要监听的 CRD 类型、版本等信息。这 个方法的最后一步是 Complete 的过程
// Complete具体过程：在构建 Controller 的方法中最重要的两个步骤是 doController 和 doWatch。在 doController 的过程中，实际的核心步骤是完成 Controller 对象的构建，从而实现基于 Scheme 和 Controller 对象的 CRD 的监听流程。而在构建 Controller 的过程中，它的 do 字段实际对应的是 Reconciler 接口类型定义的方法，也就是在 Controller 对象生成之后， 必须实现这个定义的方法。它是如何使 Reconciler 对象同 Controller 产生联系的?实际上， 在 Controller 初始化的过程中，借助了 Options 参数对象中设计的 Reconciler 对象，并将 其传递给了 Controller 对象的 do 字段。所以当我们调用 SetupWithManager 方法的时候， 不仅完成了 Controller 的初始化，还完成了 Controller 监听资源的注册与发现过程（doWatch），同时 将 CRD 的必要实现方法(Reconcile 方法)进行了再现。至此，我们完成了 Controller 的 初始化分析，
func (r *AppReconciler) SetupWithManager(mgr ctrl.Manager) error {
	// 通过 ControllerManagedBy(m manager.Manager)*Builder 方法实例化一个 Builder 对象，其中传入的 Manager 提供创建 Controller 所需的依赖。
	return ctrl.NewControllerManagedBy(mgr).
		...
		// Predicate(过滤器)是 Controller.Watch 的可选参数（使用watche进行监听资源可以自定义过滤器)，用于过滤事件（哪些事件需要进行处理）。其接口与部分 实现在 pkg/predicate/predicate.go 下。接口见代码清单 3-35，4 个方法分别对应 4 种类 型的事件过滤，如果通过过滤，则返回 True。
		...
		// }
		// Controller-runtime 内置了 5 种 Predicate 的实现
		...
		// GenerationChangedPredicate 会跳过资源对象 metadata.generation 未改变的事件。 当对对象的 Spec 字段进行写操作时，Kubernetes API 服务器会累加对象的 metadata. generation 字段。因此，GenerationChangedPredicate 允许 Controller 忽略 Spec 未更改 而仅元数据 Metadata 或状态字段 Status 发生更改的更新事件。需要注意的是，对于开发 者定义的 CRD，只有当开启了状态子资源时，metadata.generation 字段才会增加。
		// 上面提到的仅在写入 Spec 字段时 metadata.generation 字段才增加的情况，并不 适用于所有的 API 对象，例如 Deployment 对象，在写入 metadata.annotationss 时， metadata.generation 也会增加。另外，由于使用了此 Predicate，Controller 的同步不会 被只包含状态(Status)更改的事件触发，因此，无法用于同步或恢复对象的状态值。
		// (4)AnnotationChangedPredicate:只实现了 Update 事件的过滤，此 Predicate 跳过 对象的 Annotations 字段无变化的更新事件，可以与 GenerationChangedPredicate 一起使 用，用于同时需要响应对象 Spec 与 Annotation 字段更新的 Controller
		// 5)LabelChangedPredicate:只实现了 Update 事件的过滤，跳过标签(Label)未改 变的 Update 时间，也可以和上面 AnnotationChangedPredicate 一样，结合 Generation- Changed Predicate 用于同时响应对象 Spec 与 Label 字段更新的 Controller。
		// 除以上 5 种 Predicate 外，还有两个代表逻辑运算符的方法——Or() 与 And()，两个方 法可以传递多个 Predicate 接口，最终返回一个 Predicate，代表逻辑运算结果。
		// 从上面的例子可以看到，Predicate 可以通过 Controller 的 Watch() 方法设置。另 外，也可以在创建 Controller 时，将 Predicate 通过 Builder.WithEventFilter() 传递到 Controller 中，或是通过 pkg/builder/options.go 下的 WithPredicates() 方法，转换成 实现 ForOption、OwnsOption、WatchesOption 接口的 builder.Predicates 结构，在 Buil- der.For()、Builder.Owns()、Builder.Watches() 方法中设置。
		// Predicate 主要有以下特性。
		...
		// EventHandler(事件句柄)是 Controller.Watch 的参数，当事件产生时，EventHandler 将返回对象的 Name 和 Namespace，作为 Request 被添加到待处理事件队列中。例如， 将来自 Source 的 Pod Create 事件提供给 EnqueueHandler，EventHandler 将生成一个 Request 添加到队列中，这个 Request 包含该 Pod 的 Name 和 Namespace。
