...
func main() {
	...
	// Manager 初始化是借助于 ctrl.NewManager 方法实现的，而这个方法的位置指向 Controller-runtime 包的 manager.New 方法，在 New 的方法中，实际是根据传入的参数 进行 Manager 对象的 Scheme、Cache、Client 等模块的初始化构建 ctrl.GetConfigOrDie() 集群配置文件句柄，可以创建 client ， Scheme已经初始化，自建的类型和默认类型都已经初始化
	// Manager 的 New 方法中的 Scheme 变量，是借助 Kubebuilder 工具，根据用户 CRD 生成主程序 main.go 的入口函数 main 方法传入的，即这里的 Scheme 已经绑定了用户 CRD。然后，初始化 Manager 对象 构建出来后，通过 Manager 的 Cache 监听 CRD，一旦 CRD 在集群中创建了，Cache 监听 到发生了变化，就会触发 Controller 的协调程序 Reconcile 工作
	...
	// cluster.Cluster 还提供了 SetFields() 接口，用于“注入”Controller 的依赖。此接口 在创建 Controller 时作为函数对象保存在 Controller 中，在 Controller 启动前调用。
	...
	// (2)[]Runnable 类型成员 leaderElectionRunnables 与 nonLeaderElectionRunnables: 两者存储了所有注册到 Manager 中的 Controller、WebHook Server 以及自定义对象，按 照是否需要遵循选举机制 []Runnable 类型成员分为两类，nonLeaderElectionRunnables 中 的 Runnable，在调用 Manager.Start() 方法后会立即启动。
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		// (10)EventBroadcaster 参数用于提供 Manager，以获取 EventRecorder，当前已不 推荐使用，因为当 Manager 或 Controller 的生命周期短于 EventBroadcaster 的生命周期时， 可能会导致 goroutine 泄露。
		Scheme: scheme,
	...
	if err = (&controller.AppReconciler{
		// 将 Manager 的 Client 传给 Controller，
	...
	// go cm.startNonLeaderElectionRunnables()根据是否需要选举来选择启动方式 选举和非选举方式的启动逻辑类似，都是先初始化 Cache，再启动 Controller
	// 启动Cache   cm.waitForCache
	// 启动Controller for _, c := range cm.nonLeaderElectionRunnables
	// Cache 的核心逻辑是初始化内部所有的 Informer，初始化 Informer 后就创建了 Reflector 和内部 Controller，Reflector 和 Controller 两个组件是一个“生产者—消费者” 模型，Reflector 负责监听 APIServer 上指定的 GVK 资源的变化，然后将变更写入 delta 队列中，Controller 负责消费这些变更的事件，然后更新本地 Indexer，最后计算出是创建、 更新，还是删除事件，推给我们之前注册的 Watch Handler（client-go中的事件处理方法 ）
	// (2)Manager.Start() 方法会启动所有注册到 Manager 中的 Controller。当开启了 Manager 的选举功能时，Manager 会在启动前尝试获取 Leader，只有当选 Leader 成功， Manager 才会启动注册的 Controller。
	// 除了 Controller 外，开发者可以通过 Manager.Add(Runnable)方法注册自定义的对象， 例如，注册一个 HTTP Server，只需要自定义的对象实现 Runnable 接口的 Start(context. Context)error() 方法即可。一般在通过 pkg/builder 下的 Builder 创建 Controller 对象时， Builder 会自动调用 Manager.Add(Runnable)方法将 Controller 对象注册到 Manager 中。
	// 与 Controller 相同，在调用 Manager.Start() 后，Manager 会调用自定义对象的 Start(context.Context)error() 方法，用来启动自定义对象。当自定义对象同时实现了 LeaderElectionRunnable 接口的 NeedLeaderElection() 方法时，Manager 会在启动 前判断此自定义对象是否需要遵循选举机制来启动，在默认情况下，对于未实现此接口 的自定义对象，其效果与实现了此接口且返回为 True 时一样。
